...
if ($events->have_posts()):
	...
	echo '<div id="schedule-day" class="fg-schedule-block">';
		...
		while($events->have_posts()){
			...
			$event_url = $post->guid;
			$event_excerpt = ($post->post_excerpt ? $post->post_excerpt : forgivenTruncate($post->post_content,155).' ...');
			
			if ($event_all_day){
				$date_format = __('All day','forgiven');
				...
				if ($event_start_day == $event_end_day){
					...
					$date_format = __('Starts today','forgiven').' @ '.date($time_format,strtotime($event_start_time));
				} else if ($event_end_day == $todays_date_alt) {
					$date_format = __('Ends today','forgiven').' @ '.date($time_format,strtotime($event_end_time));
				} else {
					$date_format = __('All day','forgiven');
				}
			}
			...
			echo '<div class="fg-event">';
				...
				echo '<p><a class="fg-event-link" href="'.get_permalink($event_id).'">'.__('More Information','forgiven').'</a>';
	...
	echo '<h3 class="fg-schedule-none">'.__('No events scheduled for this day.','forgiven').'</h3>';
