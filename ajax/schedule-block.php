...
echo '<div class="fg-schedule-tabs">';
	...
	$date_first = date_i18n(get_option('date_format'),$day['start']);
	$date_last = date_i18n(get_option('date_format'),$day['end']);
	...
	do {
		...
		echo '<a href="#" data="'.$date_key.'" class="fg-schedule-tab'.($date_key == $todays_date || isset($_POST['first_day']) && !$days_done && $day_checker_1 != $day_checker_2 ? ' active' : '').'">';
			echo '<span class="date">'.date_i18n('F j',strtotime($current_day)).'</span>';
			echo '<span class="day">'.date_i18n('l',strtotime($current_day)).'</span>';
			echo '<span class="count'.(!$event_count ? ' empty' : '').'">'.$event_count.' '. _n('event','events',$event_count,'forgiven').'</span>';
		echo '</a>';
