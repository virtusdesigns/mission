...
function page_array(){
	...
	$pages[''] = __('Select a page...','forgiven');
...
function post_array($type = 'post'){
	...
	if ($type_name == 'soliloquy') { /* Do nothing */ } else if ($type_name == 'forgiven'){ $posts[0] = 'No slider'; } else { $posts[0] = __('Select','forgiven').' '.$a_text.' '.$type_name.$post_text.' ...'; }
	
	foreach ($posts_obj as $post) {
		if ($type_name == 'forgiven'){
			$posts['FORGIVENSLIDE---'.$post->ID] = 'FORGIVEN: '.$post->post_title;
